...
import { ArrowLeft, Save, Loader2, Upload, X, Plus, Trash2, Clock, Calendar, Armchair } from "lucide-react";
import { getMovieAPI, updateMovieAPI, type SeatRow } from "@/lib/api";
...
function EditMovieContent() {
  ...
  const [form, setForm] = useState({
    ...
    endDate: "",
    showtimes: ["10:00", "14:00", "18:00", "22:00"] as string[],
    seatConfig: [] as SeatRow[],
  });
  ...
  useEffect(() => {
    ...
    const fetchMovie = async () => {
      try {
        const movie = await getMovieAPI(movieId);
        if (movie) {
          setForm({
            title: movie.title,
            ...
            showtimes: movie.showtimes || ["10:00", "14:00", "18:00", "22:00"],
            seatConfig: movie.seatConfig || [
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateMovieAPI(movieId, form);
      router.push("/admin/dashboard");
    } catch {
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] text-white flex items-center justify-center">
        <div className="text-center">
          <Loader2 size={32} className="animate-spin mx-auto mb-4 text-red-600" />
          <p>Loading movie...</p>
        </div>
      </div>
    );
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="border-t border-white/10 pt-6">
              ...
              <div className="space-y-4">
                {form.seatConfig.map((row, idx) => (
                  <div key={idx} className="flex items-center gap-3 bg-white/5 rounded-lg p-4">
                    <input
                      type="text"
                      value={row.rowId}
                      onChange={(e) => updateSeatRow(idx, "rowId", e.target.value)}
                    ...
                    <input
                      type="text"
                      value={row.label}
                      onChange={(e) => updateSeatRow(idx, "label", e.target.value)}
                    ...
                    <input
                      type="number"
                      value={row.price}
                      onChange={(e) => updateSeatRow(idx, "price", parseInt(e.target.value) || 0)}
                    ...
                    <input
                      type="number"
                      value={row.seats}
                      onChange={(e) => updateSeatRow(idx, "seats", parseInt(e.target.value) || 0)}
